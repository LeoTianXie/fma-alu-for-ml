...
module fp32_accumulator_tb;
    ...
    task automatic calc_shift_grs(
        ...
        input  logic        in_guard,
        input  logic        in_round,
        input  logic        in_sticky,
        ...
        logic [27:0] in_ext;
        logic [27:0] shifted_ext;
        in_ext        = {in_man, in_guard, in_round, in_sticky};
        shifted_ext   = 28'b0;
        r_man_aligned = 25'b0;
        r_guard       = 1'b0;
        r_round       = 1'b0;
        ...
        if (shift_amount <= 8'd27) begin
            for (int i = 0; i < 28; i++) begin
                if ((i + shift_amount) < 28) begin
                    shifted_ext[i] = in_ext[i + shift_amount];
                end
            end

            r_man_aligned = shifted_ext[27:3];
            r_guard       = shifted_ext[2];
            r_round       = shifted_ext[1];
            r_sticky      = shifted_ext[0];

            for (int i = 0; i < 28; i++) begin
                if ((shift_amount != 8'd0) && (i < shift_amount)) begin
                    r_sticky = r_sticky | in_ext[i];
                end
            end
        end else begin
            r_sticky = |in_ext;
    ...
    task automatic ref_model(
        ...
        input  logic        r_acc_guard_in,
        input  logic        r_acc_round_in,
        input  logic        r_acc_sticky_in,
        ...
        calc_shift_grs(r_acc_man, r_acc_guard_in, r_acc_round_in, r_acc_sticky_in,
                       acc_shift_amount, acc_man_shifted,
    ...
    task automatic check(input string label);
        ...
        logic [7:0]  r_exp;
        logic [24:0] r_man;
        logic        r_guard, r_round, r_sticky, r_carry;

        ref_model(sign_p, man_aligned, common_exp, guard_bit, round_bit, sticky_bit,
                  acc_sign, acc_exp, acc_man, acc_guard_in, acc_round_in, acc_sticky_in,
                  r_sign, r_exp, r_man, r_guard, r_round, r_sticky, r_carry);

        if (sign_acc !== r_sign || exp_acc !== r_exp || man_acc !== r_man ||
            guard_acc !== r_guard || round_acc !== r_round || sticky_acc !== r_sticky ||
            carry_acc !== r_carry) begin
            $display("FAIL [%-38s] p=(s=%b man=%025b exp=%0d grs=%b%b%b) acc=(s=%b man=%025b exp=%0d grs=%b%b%b) | got s=%b exp=%0d man=%025b grs=%b%b%b c=%b | exp s=%b exp=%0d man=%025b grs=%b%b%b c=%b",
                     label,
                     sign_p, man_aligned, common_exp, guard_bit, round_bit, sticky_bit,
                     acc_sign, acc_man, acc_exp, acc_guard_in, acc_round_in, acc_sticky_in,
                     sign_acc, exp_acc, man_acc, guard_acc, round_acc, sticky_acc, carry_acc,
                     r_sign, r_exp, r_man, r_guard, r_round, r_sticky, r_carry);
            fail_count++;
        end else begin
            pass_count++;
    ...
    task automatic apply_and_check(
        ...
        input logic        t_acc_guard_in = 1'b0,
        input logic        t_acc_round_in = 1'b0,
        input logic        t_acc_sticky_in = 1'b0
    );
        sign_p      = t_sign_p;
        ...
        acc_exp     = t_acc_exp;
        acc_man     = t_acc_man;
        acc_guard_in = t_acc_guard_in;
        acc_round_in = t_acc_round_in;
        acc_sticky_in = t_acc_sticky_in;
        #1;
        check(label);
    endtask
    ...
    initial begin
        ...
        // new positive add generates the explicit carry_acc bit for top-level renorm.
        apply_and_check("same sign saturated bit24 sets carry_acc",
                        1'b0, 25'h1000000, 8'd130, 1'b0, 1'b0, 1'b0,
                        1'b0, 8'd130, 25'h1000000);

        apply_and_check("same sign saturated bit24 plus low carry sets carry_acc",
                        1'b1, 25'h1FFFFFF, 8'd130, 1'b0, 1'b0, 1'b0,
                        1'b1, 8'd130, 25'h0000001);

        apply_and_check("same sign bit24 + c2 sets carry_acc",
                        1'b0, 25'h0FFFFFF, 8'd127, 1'b0, 1'b0, 1'b0,
        ...
        apply_and_check("acc stored guard shifts into round/sticky path",
                        1'b0, 25'h0000000, 8'd128, 1'b0, 1'b0, 1'b0,
                        1'b0, 8'd127, 25'h0000000, 1'b1, 1'b0, 1'b0);

        apply_and_check("acc stored round/sticky survive later align",
                        1'b0, 25'h0000000, 8'd129, 1'b0, 1'b0, 1'b0,
                        1'b0, 8'd127, 25'h0000000, 1'b0, 1'b1, 1'b1);
